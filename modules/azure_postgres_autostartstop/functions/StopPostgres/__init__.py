...
def main(stoppostgrestimer: func.TimerRequest) -> None:
    ...
    try:
        ...
        poller.result()
        logging.info(f"Successfully stopped PostgreSQL Flexible Server {POSTGRES_SERVER_NAME}: {POSTGRES_RESOURCE_GROUP}...")
    except Exception as err:
        logging.error(f"Error while stopping PostgreSQL Flexible Server {POSTGRES_SERVER_NAME}: {err}")

        trigger_slack_alert(
            server_name=POSTGRES_SERVER_NAME,
            resource_group=POSTGRES_RESOURCE_GROUP,
            action="Stop Postgres Flexible Server",
